...
function Instancias() {
    ...
    const loadInstances = async () => {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                const sortedInstances = data.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    ...
    const handleCreateInstance = async (instanceName) => {
        ...
        try {
            ...
            if (error.response && error.response.status === 400) {
                setModalError(error.response.data.message || 'Error al crear la instancia.');
            } else {
                setModalError('Ocurrió un error al crear la instancia. Inténtalo nuevamente.');
            }
        } finally {
    ...
    const handleModalClose = () => {
        setIsModalOpen(false);
        setModalError('');
        loadInstances(); // Llamamos a loadInstances al cerrar el modal
    };
    ...
    const filteredInstances = instances.filter(instance => {
        ...
        const matchesStatus = statusFilter === 'Todos' ||
            ...
            (statusFilter === 'Desconectado' && (instance.connectionStatus === 'close' || instance.connectionStatus === 'connecting'));
            ...
            <div className="instance-grid">
                {!loading && filteredInstances.map(instance => (
                    <InstanceCard
                        ...
                        phone={instance.ownerJid ? instance.ownerJid.split('@')[0] : "Sin número"}
                        showQr={instance.connectionStatus === 'close' || instance.connectionStatus === 'connecting'}
